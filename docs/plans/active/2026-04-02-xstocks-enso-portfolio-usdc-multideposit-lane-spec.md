...
5. The repo now also includes a Privy-free direct probe at [enso-direct-quoteability-diagnostic.js](/Users/user/PycharmProjects/xstocks-strategy-lab/apps/api/scripts/enso-direct-quoteability-diagnostic.js). The latest direct proof [summary.json](/Users/user/PycharmProjects/xstocks-strategy-lab/tmp/proof/enso-direct-diagnostic-2026-04-05T12-04-48.737Z/summary.json) proves:
   - `wallet/approve` still succeeds,
   - `NVDAx` and `AMZNx` are routeable at the promoted-basket notionals through both `USDC` and `USDG`, and through both deployment-address and wrapper-address outputs,
   - a real smart-account `delegate` bundle over `NVDAx + AMZNx` returns executable tx calldata,
...
6. `AUSD` still lacks the Ethereum execution/payment metadata required for this lane, and the current boundary package intentionally surfaces it as a bridge helper with null Ethereum execution addresses, so the exact promoted basket remains unproven and this lane must not replace hosted `1inch` as the public-default route.
