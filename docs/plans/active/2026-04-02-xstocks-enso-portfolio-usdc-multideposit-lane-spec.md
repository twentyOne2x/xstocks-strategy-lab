...
5. The repo now also includes a Privy-free direct probe at [enso-direct-quoteability-diagnostic.js](/Users/user/PycharmProjects/xstocks-strategy-lab/apps/api/scripts/enso-direct-quoteability-diagnostic.js). The latest direct proof [summary.json](/Users/user/PycharmProjects/xstocks-strategy-lab/tmp/proof/enso-direct-diagnostic-2026-04-04T08-54-34.041Z/summary.json) re-proves that `wallet/approve` succeeds, `NVDAx` and `AMZNx` quote at the real promoted-basket notionals, receiver choice does not rescue the failing names, and `MSFTx`, `AAPLx`, `METAx`, and `GOOGLx` still fail direct Enso routing even when swept up through `$1000`.
