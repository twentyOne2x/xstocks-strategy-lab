...
8. The repo now also carries a Privy-free direct Enso substrate diagnostic at [enso-direct-quoteability-diagnostic.js](/Users/user/PycharmProjects/xstocks-strategy-lab/apps/api/scripts/enso-direct-quoteability-diagnostic.js). The latest direct proof [summary.json](/Users/user/PycharmProjects/xstocks-strategy-lab/tmp/proof/enso-direct-diagnostic-2026-04-05T12-04-48.737Z/summary.json) proves:
   - `wallet/approve` still returns a real `USDC` approval transaction,
   - `NVDAx` and `AMZNx` are routeable at the real promoted-basket notionals through both `USDC` and `USDG`,
   - those same two names also route through both deployment-address and wrapper-address outputs,
   - a real smart-account `delegate` bundle over `NVDAx + AMZNx` now returns executable tx calldata,
   - `MSFTx`, `AAPLx`, `METAx`, and `GOOGLx` still fail direct Enso routing across `USDC`/`USDG` funding and deployment/wrapper output variants,
   - and those four failing names still fail receiver changes plus direct sweeps through `$1000`.
9. The same direct diagnostic also re-proves the current `AUSD` package truth: the repo-owned Enso boundary intentionally surfaces `AUSD` as a bridge helper with `supportsAtomicSwaps=true` but `address=null` and `wrapperAddress=null`, so the Enso promoted-basket lane still lacks executable Ethereum token metadata for the yield-buffer sleeve.
...
| Enso bundle lane | 84% | 63% | 0% | resolve upstream quoteability for `MSFTx`, `AAPLx`, `METAx`, and `GOOGLx`, resolve `AUSD` metadata, then re-run the promoted basket and still require landed onchain proof before any live claim |
...
5. The current exact blocker is no longer missing env. The repo-owned approval path is fixed, the repo now includes a Privy-free direct diagnostic, and that diagnostic proves a real smart-account `delegate` bundle for `NVDAx + AMZNx`. The promoted basket still fails because `MSFTx`, `AAPLx`, `METAx`, and `GOOGLx` are not currently quoteable through Enso across `USDC`/`USDG` funding and deployment/wrapper output variants or at a direct sweep through `$1000`, and `AUSD` still lacks the required Ethereum/USDC metadata.
