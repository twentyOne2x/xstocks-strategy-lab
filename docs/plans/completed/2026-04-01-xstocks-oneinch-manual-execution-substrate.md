...
the repo-owned proof harness now writes a blocker bundle at [summary.json](/Users/user/PycharmProjects/xstocks-strategy-lab/tmp/proof/oneinch-fusion-2026-04-01T21-55-55.559Z/summary.json), and the exact blocker is `Privy access token is expired.` with `statusCode=401`. A fresh verified user session is still required before authenticated activation, quote, signature, or submission proof can continue.

## Canonical Execution Contract

The reusable phase-1 substrate remains the existing shared execution request and leg contract in [execution.ts](/Users/user/PycharmProjects/xstocks-strategy-lab/packages/shared/src/contracts/execution.ts).
...
1. Added [oneinch.ts](/Users/user/PycharmProjects/xstocks-strategy-lab/packages/xstocks/src/adapters/oneinch.ts) plus 1inch Fusion types in [types.ts](/Users/user/PycharmProjects/xstocks-strategy-lab/packages/xstocks/src/types.ts).
2. Extended [execution.ts](/Users/user/PycharmProjects/xstocks-strategy-lab/packages/shared/src/contracts/execution.ts) with `oneinch_fusion` quote storage and generalized request ownership/source enums.

### Backend substrate wiring

1. [api-service.js](/Users/user/PycharmProjects/xstocks-strategy-lab/apps/api/src/services/api-service.js) now resolves the manual execution venue explicitly, builds 1inch quotes, persists signer-owned approval payloads, records signed submissions, refreshes 1inch venue status, and keeps CoW as the default path.
2. [runtime-store.js](/Users/user/PycharmProjects/xstocks-strategy-lab/apps/api/src/repositories/runtime-store.js) now normalizes stored `oneinch_fusion` quotes.
3. [contracts.js](/Users/user/PycharmProjects/xstocks-strategy-lab/apps/api/src/contracts.js) now documents `executionRouteId` on the execution write surface.

### Proof and verification surfaces

1. Added [oneinch-fusion-proof.js](/Users/user/PycharmProjects/xstocks-strategy-lab/apps/api/scripts/oneinch-fusion-proof.js) to drive the authenticated manual 1inch path and emit proof artifacts.
2. Added 1inch execution-path tests to [api.test.js](/Users/user/PycharmProjects/xstocks-strategy-lab/apps/api/test/api.test.js).
...
4. `node --check apps/api/scripts/oneinch-fusion-proof.js`
5. `node apps/api/scripts/oneinch-fusion-proof.js`

Results:
1. xStocks package tests passed.
2. Shared package build passed.
3. API tests passed.
4. Proof runner syntax check passed.
5. The proof runner emitted a blocker artifact rather than crashing and stopped at the exact authenticated-session boundary.

## Exact Proof Artifact And Blocker

Artifact:
1. [summary.json](/Users/user/PycharmProjects/xstocks-strategy-lab/tmp/proof/oneinch-fusion-2026-04-01T21-55-55.559Z/summary.json)

Exact blocker:
1. `code = "proof_request_failed"`
2. `stage = "environment_or_authentication"`
3. `message = "Privy access token is expired."`
4. `statusCode = 401`

This means the remaining proof failure in the current environment is not a fabricated venue claim, not a quote-only fallback, and not a submission bug inside the new substrate. The blocker is the lack of a fresh authenticated Privy session token needed to start the signer-owned proof path at all.
