...
| Autoresearch runtime: public and local default truth are now fail-closed, but the stale Railway host-proof story and blocked Railway CLI still need reconciliation | `XSL-006` with `XSL-006A` | [2026-03-31-xstocks-strategy-lab-autoresearch-operating-model-spec.md](/Users/user/PycharmProjects/xstocks-strategy-lab/docs/plans/active/2026-03-31-xstocks-strategy-lab-autoresearch-operating-model-spec.md) | default API boot no longer auto-loads the historical Railway proof seed; the remaining gap is live host proof or explicit retirement of the stale Railway claim |
...
| Enso portfolio multi-deposit from one `USDC` into the promoted default basket: partial implementation exists, proof runner now exists, atomic bundle still not repo-proven | `XSL-005` with `XSL-005B` | [2026-03-31-xstocks-execution-funding-and-rails-spec.md](/Users/user/PycharmProjects/xstocks-strategy-lab/docs/plans/active/2026-03-31-xstocks-execution-funding-and-rails-spec.md) | approval-path bug is fixed, a real smart-account `delegate` bundle now returns tx payload for `NVDAx + AMZNx`, but no landed tx exists; current exact blocker is persistent Enso route coverage failure on `MSFTx`, `AAPLx`, `METAx`, and `GOOGLx` across `USDC`/`USDG` funding and deployment/wrapper outputs, plus intentional non-executable `AUSD` bridge metadata |
| Portfolio buy public-default route: hosted `1inch`; canonical frontend now reaches the real Brave wallet handoff, but the lane still stops at wallet unlock / signature state | `XSL-005` with `XSL-005C` | [2026-03-31-xstocks-execution-funding-and-rails-spec.md](/Users/user/PycharmProjects/xstocks-strategy-lab/docs/plans/active/2026-03-31-xstocks-execution-funding-and-rails-spec.md) | `Buy portfolio` stays public, the live frontend now reaches the real Privy modal and Brave wallet panel, but no public execution lane may be called done until a landed tx is proven through that canonical surface |
...
- Canonical owner lane: `XSL-005`
- Date opened: 2026-04-02
- Context: the same user intent now also names Enso as a candidate substrate for one-transaction multi-deposit from `USDC` into all assets of the promoted default portfolio. Current repo truth still proves only the explicit `1inch.ethereum` core-xStocks lane, but the repo now also carries partial Enso implementation across shared contracts, `apps/api`, and `apps/web` without any live proof bundle.
- Suspected cause: implementation advanced faster than proof closure. The repo already had an Enso bundle contract shape, API quote path, and manual-execution handling; the missing pieces were an executable proof harness plus operator env needed to hit the promoted basket for real.
- Fix intent: keep ownership under `XSL-005`, use the new proof runner and artifact bundle, prove the exact promoted basket including the `AUSD` sleeve, and either advance Enso to a truthful live claim or freeze it explicitly as non-live.
- Acceptance criteria:
  1. The repo explicitly keeps this lane under `XSL-005` and does not open it as a new `XSL-014` route-design sub-lane.
...
- Plan links:
  - [2026-04-02-xstocks-enso-portfolio-usdc-multideposit-lane-spec.md](/Users/user/PycharmProjects/xstocks-strategy-lab/docs/plans/active/2026-04-02-xstocks-enso-portfolio-usdc-multideposit-lane-spec.md)
  - [2026-04-03-xstocks-remaining-live-proof-and-operator-access-closure.md](/Users/user/PycharmProjects/xstocks-strategy-lab/docs/plans/active/2026-04-03-xstocks-remaining-live-proof-and-operator-access-closure.md)
- Executor prompt:
  - Work only after this spec is approved.
  - Keep ownership under `XSL-005`.
...
- Continuation note:
  ...
  - The old env/config blocker and dead approval-endpoint bug are now closed; neither is the current blocker anymore.
  - The strongest env-backed proof is still [summary.json](/Users/user/PycharmProjects/xstocks-strategy-lab/tmp/proof/enso-portfolio-2026-04-03T22-33-23.502Z/summary.json), which reaches `quote_portfolio` and records the promoted-basket bundle failure instead of failing on local config.
  - The strongest Privy-free substrate proof is now [summary.json](/Users/user/PycharmProjects/xstocks-strategy-lab/tmp/proof/enso-direct-diagnostic-2026-04-05T12-04-48.737Z/summary.json). It proves `wallet/approve` succeeds, `NVDAx` and `AMZNx` quote at the real promoted-basket notionals, a real smart-account `delegate` bundle over `NVDAx + AMZNx` returns executable tx calldata, and `MSFTx`, `AAPLx`, `METAx`, and `GOOGLx` still fail Enso routing across `USDC`/`USDG` funding and deployment/wrapper output variants while `AUSD` remains intentionally metadata-blocked.
  - The next honest move is still not a route switch. It is either an upstream Enso-routing resolution for the four failing promoted legs plus executable `AUSD` metadata, or an explicit freeze of Enso as non-canonical while hosted `1inch` remains the public default.
...
- Acceptance criteria:
  ...
  3. Railway operator access is restored or a deliberate non-CLI audit path is documented and exercised.
  4. The current public runtime API remains fail-closed unless a fresh live host receipt is captured.
  5. If fresh host proof still cannot be produced, the stale local rehydration path is retired by default and one exact blocker or retirement decision is recorded instead of preserving stale “completed” language.
- Complexity: medium
- Plan links:
...
- Executor prompt:
  ...
  - Audit the current host claim first: `railway whoami`, service inventory, live runtime API, and the checked-in proof seed all need to be reconciled together.
  - Prefer reviving or re-verifying the existing Railway worker surface before considering any new scheduler host.
  - If Railway CLI remains blocked, use one deliberate non-CLI audit path instead of hand-waving the lane closed.
- Checklist:
  - [x] report captured
  - [x] context added
  - [x] fix applied
...
- Recovery resolution note:
  ...
  - 2026-04-04: `GET https://24-7.markets/api/runtime/autoresearch?limit=1` now returns `truthBoundary=railway_cron_service`, `recurringAutonomousProven=true`, and `schedulerHost.serviceName=autoresearch-worker`; local default boot remains fail-closed unless an explicit proof path is supplied.
...
- Complexity: medium
- Plan links:
  - [2026-04-02-xstocks-oneinch-hosted-session-backed-signer-proof.md](/Users/user/PycharmProjects/xstocks-strategy-lab-xsl014-proof/docs/plans/active/2026-04-02-xstocks-oneinch-hosted-session-backed-signer-proof.md)
  - [2026-04-03-xstocks-remaining-live-proof-and-operator-access-closure.md](/Users/user/PycharmProjects/xstocks-strategy-lab/docs/plans/active/2026-04-03-xstocks-remaining-live-proof-and-operator-access-closure.md)
- Executor prompt:
...
- Verification note:
  ...
  - An earlier same-day rerun briefly stopped at `code=proof_request_failed`, `stage=environment_or_authentication`, and `message="Privy access token is expired."`
  - A later rerun from the current repo state verified fresh access-token auth and wrote [summary.json](/Users/user/PycharmProjects/xstocks-strategy-lab/tmp/proof/oneinch-fusion-2026-04-03T23-13-29.688Z/summary.json), [approval-payloads.json](/Users/user/PycharmProjects/xstocks-strategy-lab/tmp/proof/oneinch-fusion-2026-04-03T23-13-29.688Z/approval-payloads.json), and [signature-inputs.json](/Users/user/PycharmProjects/xstocks-strategy-lab/tmp/proof/oneinch-fusion-2026-04-03T23-13-29.688Z/signature-inputs.json).
  - That latest rerun recreated fresh `activationId` and `executionRequestId`, confirmed six approval payloads plus six signature inputs, and stopped at the same exact blocker: `code=missing_user_signature`, `stage=awaiting_signature`. No signed submission, venue order id, tx hash, or receipt truth exists yet.
  - The next integration surface is no longer auth refresh. It is signer injection into the already-landed proof-runner inputs via `XSTOCKS_ONEINCH_ORDER_SIGNATURES_JSON` or `XSTOCKS_ONEINCH_ORDER_SIGNATURES_PATH`, followed by exactly one rerun.
- Continuation note:
  - Date: 2026-04-03 remaining-gap closure pass
  ...
  - The shortest truthful next move is now: consume the already-captured signer packet or `signature-inputs.json`, inject the six signer-owned EIP-712 signatures through `XSTOCKS_ONEINCH_ORDER_SIGNATURES_JSON` or `XSTOCKS_ONEINCH_ORDER_SIGNATURES_PATH`, and rerun once. The lane still must not be widened into a new funding/custody diagnosis until that rerun advances past the signer boundary again.
